...
function restructureChatBoxes() {
  ...
  function checkMessages() { 
                ...
                $.ajax({
				url : "<?php echo $CONFIG->wwwroot; ?>mod/the_chat/chat.php?action=rx", 
...
//	            alert("no messages");
	...
	if(event.keyCode == 40 && bb_chat_messages.length != 0) {
	  ...
	  if (pos >= bb_chat_messages[chatboxtitle].length) {
        ...
        if(event.keyCode == 13 && event.shiftKey == 0)  {
                ...
                $.ajax({
				url : "<?php echo $CONFIG->wwwroot; ?>mod/the_chat/chat.php?action=tx", 
	...
	ind = text.indexOf("*");
      ...
      for (var i in smiley_array) {
        var smiley_img = '<img alt="" src="<?php echo $CONFIG->wwwroot; ?>mod/the_chat/graphics/icons/smilies/' + smiley_xhtml[i] + '" />';
